...
class Forecasting_of_covid_cases():
    ...
    def Predict_cases(self):
        ...
        self.final_prediction['Future Dates']=[str(value) for value in self.daterange]
...
class Forecasting_of_covid_cases_for_state(Forecasting_of_covid_cases):
    def __init__(self,type_of_case,state_id):
        with open('./Main_Workspace/config.env') as f:
            credentials=f.read()
        credentials=credentials.split(" ")
        con = mysql.connector.connect(username=credentials[0],password=credentials[1],host=credentials[2],port=credentials[3],database=credentials[4])
        query=con.cursor()
        sql="SELECT * FROM total_state_cases WHERE state_name='{}'".format(state_id)
        query.execute(sql)
        result=query.fetchall()
        column=[columns[0] for columns in query.description]
        state_data=pandas.DataFrame(result,columns=column)
        order_of_forecasting_models1=(5,0,3)
        order_of_forecasting_models2=(3,0,3)
        super().__init__(state_data,state_id,order_of_forecasting_models1,order_of_forecasting_models2,type_of_case)
...
if __name__=="__main__":
    ...
    maharashtra_class=Forecasting_of_covid_cases_for_state('delta_confirmed','MH')
    print(maharashtra_class.Predict_cases())
